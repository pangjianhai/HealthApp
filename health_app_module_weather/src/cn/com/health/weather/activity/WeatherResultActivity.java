...
public class WeatherResultActivity extends Activity {
	/**
	 * 服务器返回的天气数据
	 */
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		dialog = new ProgressDialog(WeatherResultActivity.this);
	 ...
	 * @user:pang
	 * @data:2015年6月1日
	 * @todo:访问天气预报服务器获取json格式数据
	 * @return:JSONObject
	 */
	public JSONObject getWeatherData(String citycode) throws Exception {
	 ...
	 * @user:pang
	 * @data:2015年6月1日
	 * @todo:解析服务器返回的JSON数据,并将数据保存到对应的String变量中
	 * @return:void
	 */
	public void getWeatherInfo(String citycode) {
		try {
			...
			mHandler.post(new Runnable() {
				...
				public void run() {
					...
					Toast.makeText(WeatherResultActivity.this,
							"无法获取数据,改用之前的数据", Toast.LENGTH_SHORT).show();
				}
			});
			// 当发生异常无法正常获取天气信息数据时,改用之前保存的天气信息数据
			// 争取每一次最新的网络存储的数据都写入到手机存储空间
			...
			if (all_weather_info != null) {
				...
				mHandler.post(new Runnable() {
					...
					public void run() {
						// TODO Auto-generated method stub
						Toast.makeText(WeatherResultActivity.this, "没有保存的数据",
		...
		if (weather_info_json != null) {
			try {
				...
				// 当天气信息中的时间值与保存在xml中时间值不同时就保存当前天气信息到xml以备无网络情况下使用，也为了提高效率
				if (!date_y.equals(getDateFromXml())) {// 首先判断有没有将今天的某个城市的天气数据写入手机存储
					// 如果没有就写入
	 ...
	 * @user:pang
	 * @data:2015年6月1日
	 * @todo:根据给定日期推算出addday之后的日期
	 * @return:String
	 */
	public String getDateTime(String basetime, int addday) {
		Date date = null;
	 ...
	 * @user:pang
	 * @data:2015年6月1日
	 * @todo:保存天气信息到/data/data/packagename/shared_prefs/目录下
	 * @return:void
	 */
	public void saveInfo2Xml(String weatherinfo, String city, String citycode,
			String date) {
	 ...
	 * @user:pang
	 * @data:2015年6月1日
	 * @todo:从/data/data/packagename/shared_prefs/目录下读取保存的天气信息
	 * @return:String
	 */
	public String getInfoFromXml(String citycode) {
		String spName = getPackageName() + "_weatherinfo";
		SharedPreferences sp = getSharedPreferences(spName, MODE_PRIVATE);
	 ...
	 * @user:pang
	 * @data:2015年6月1日
	 * @todo:从/data/data/packagename/shared_prefs/目录下读取保存的信息中的date值
	 * @return:String
	 */
